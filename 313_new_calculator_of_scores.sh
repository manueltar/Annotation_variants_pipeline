...
Rscript_COGS=/nfs/users/nfs_m/mt19/Scripts/R/335_COGS.R
...
#echo "bsub -G team151 -o $outfile_Prepared_files_and_add_annotation_layer -M $step_mem -w\"done($name_binder_of_scores_GENE_PLOEUF)\" -J $name_Prepared_files_and_add_annotation_layer -R\"select[model==Intel_Platinum]\" -R\"select[mem>=$step_mem] rusage[mem=$step_mem] span[hosts=1]\" -n$step_pc -q $queue -- \\" >> $output
