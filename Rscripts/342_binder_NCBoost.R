...
Data_wrangling_NCBoost = function(option_list)
{
  opt_in = option_list
  opt <<- option_list
  ...
  ### Read the NCBoost_result ----
  
 
  
  NCBoost_result = as.data.frame(fread(opt$NCBoost_result, sep="\t", header = T) ,stringsAsFactors=F)
  
  NCBoost_result$VAR<-paste(paste('chr',NCBoost_result$chr, sep=''),NCBoost_result$pos,NCBoost_result$ref,NCBoost_result$alt,sep="_")
  
  
  cat("NCBoost_result_0\n")
  cat(str(NCBoost_result))
  cat("\n")
  cat(str(unique(NCBoost_result$VAR)))
  cat("\n")
  ...
  indx.int<-c(which(colnames(Merge_table) == "VAR"),which(colnames(Merge_table) == "closest_gene_name"),which(colnames(Merge_table) == "NCBoost"))
  ...
  Merge_table_subset_NO_NA<-Merge_table_subset[!is.na(Merge_table_subset$NCBoost),]
  ...
  write.table(Merge_table_subset_NO_NA, file="NCBoost_GLOBAL.tsv", sep="\t", quote = F, row.names = F)
  
  write.table(check, file="check_NCBoost_GLOBAL.tsv", sep="\t", quote = F, row.names = F)
...
main = function() {
  ...
  option_list <- list(
    ...
    make_option(c("--NCBoost_result"), type="character", default=NULL, 
                metavar="type", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
