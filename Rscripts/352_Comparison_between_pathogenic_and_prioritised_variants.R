...
Comparison_Z_score = function(option_list)
{
  opt_in = option_list
  opt <<- option_list
  ...
  if(length(Pivotal_variety) >1)
  {
    ...
    if(dim(PW_category_NO_NA)[1] > 0)
    {
      ...
      write.table(PW_category_NO_NA, file=paste("value_Z_score_Pathogenic_Comparison_Absolute_Effect_size_Z_score_with_stats",'_',finemap_prob_Threshold,'.tsv', sep=''), 
...
Comparison_value = function(option_list)
{
  opt_in = option_list
  opt <<- option_list
  ...
  Summary_table<-as.data.frame(GWAS_GLOBAL_per_traits_subset.dt[,.(n=.N,
                                                                   Min=round(as.numeric(summary(value)[1]),3),
                                                                   Q1=round(as.numeric(summary(value)[2]),3),
                                                                   M=round(as.numeric(summary(value)[3]),3),
                                                                   Q3=round(as.numeric(summary(value)[5]),3),
                                                                   Max=round(as.numeric(summary(value)[6]),3)),by=key(GWAS_GLOBAL_per_traits_subset.dt)], stringsAsFactors=F)
  ...
  if(length(Pivotal_variety) >1)
  {
    ...
    PW_category<-pairwise.wilcox.test(GWAS_GLOBAL_per_traits_subset$value, GWAS_GLOBAL_per_traits_subset[,indx.category_INTRO],
                                      p.adjust.method = "BH")
    ...
    if(dim(PW_category_NO_NA)[1] > 0)
    {
      ...
      write.table(PW_category_NO_NA, file=paste("value_Pathogenic_Comparison_Absolute_Effect_size_Z_score_with_stats",'_',finemap_prob_Threshold,'.tsv', sep=''), 
                  sep="\t",quote=F, row.names = F)
...
main = function() {
  ...
  Comparison_Z_score(opt)
  Comparison_value(opt)
}
